...
const api_url =
  process.env.API_URL || "http://api.venerispro.bitiland.com/api/";
...
router.post("/register", async function (req, res) {
  ...
  await axios
    .post(api_url + "client/", JSON.stringify(userObj), {
      headers: {
        "content-type": "application/json",
      },
    })
...
router.post("/changePassword", verifyToken, async (req, res) => {
  if (req.decode) {
    ...
    await axiosInstance
      .post(api_url + "auth/changepassword", req.body)
      .then((updateRes) => {
...
router.get("/lawyerList", checkAuth, async function (req, res) {
  if (req.query.lawyerID) {
    ...
    var url = encodeURI(
      api_url + "lawyer/search/byNameRex/?key=" + req.query.NameRex
    );
...
router.get("/lawyer/searchByName/:key", async (req, res) => {
  if (req.params.key) {
    ...
    var url = encodeURI(
      api_url + "lawyer/search/byNameRex/?key=" + req.params.key
    );
    ...
    await axios
      ...
      .then(function (responseArr) {
        if (Array.from(responseArr.data).length > 0) {
          ...
          res
            .status(500)
            .json({ message: "Không tồn tại tên luật sư đã nhập!" });
        }
      })
      .catch(function (error) {
        console.log(error);
...
router.get("/post-detail/:PostId", async function (req, res) {
  let postDetail = api_url + "post/" + req.params.PostId;
  let allPost = api_url + "post";
  let allLawyer = api_url + "lawyer/";
  const requestPostDetail = axios.get(postDetail);
  const requestAllPost = axios.get(allPost);
  const requestAllLawyer = axios.get(allLawyer);

  axios
    .all([requestPostDetail, requestAllPost, requestAllLawyer])
    .then(
      axios.spread((...responses) => {
        const requestPostDetail = responses[0].data;
        const requestAllPost = responses[1].data;
        const requestAllLawyer = responses[2].data;
        res.render("client/post-detail", {
          Post: requestPostDetail,
          postList: requestAllPost,
          lawyer_list: requestAllLawyer,
        });
      })
    )
    .catch((errors) => {
  ...
  //   await axios
  //     .get(api_url + "post/" + req.params.PostId, {
  //       headers: {
  //         "content-type": "application/json",
  //       },
  //     })
  ...
  //       if (
  //         postResponse.data.AdminID == null ||
  //         postResponse.data.AdminID == ""
  //       ) {
  //         res.render("client/post-detail", {
  ...
  //       await axios
  //         .get(api_url + "admin/" + postResponse.data.AdminID, {
  //           header: {
  //             "content-type": "application/json",
  //           },
  //         })
  ...
  await axios
    ...
    .then(function (response) {
      ...
      axios
        .all([
          axiosInstance.post(
            api_url + "lawyer/" + response.data.LawyerID + "/formation",
            lawyerFormation
          ),
          axiosInstance.post(
            api_url + "lawyer/" + response.data.LawyerID + "/experience",
            lawyerExperience
          ),
        ...
        .catch(function (error) {
          console.log(
            "Registration formation/experience/domain unsuccessfully"
          );
          console.log(error);
        });
...
router.get("/lawyer/profile", verifyToken, async function (req, res) {
  if (req.decode) {
    if (req.query.lawyerID) {
      await axios
        .get(api_url + "lawyer/" + req.query.lawyerID)
        .then(function (responseArr) {
          console.log(responseArr.data);
...
router.get("/lawyer/dashboard", verifyToken, async function (req, res) {
  ...
  if (req.decode) {
    if (req.query.UserID) {
      ...
      await axiosInstance
        .get(api_url + "lawyer/" + req.query.UserID)
        .then(function (responseArr) {
          console.log(responseArr.data);
...
router.put(
  "/lawyer/dashboard/setting/:LawyerID",
  verifyToken,
  async function (req, res) {
    if (req.decode) {
      ...
      if (req.params.LawyerID) {
        ...
        await axiosInstance
          .put(api_url + "lawyer/" + req.params.LawyerID, req.body)
          .then(function (responseArr) {
...
router.put(
  "/lawyer/dashboard/contact/:AddressID",
  verifyToken,
  async function (req, res) {
    if (req.decode) {
      console.log("Updating...");
      if (req.params.AddressID) {
        ...
        await axiosInstance
          .put(api_url + "address/" + req.params.AddressID, req.body)
          .then(function (responseArr) {
            console.log(responseArr);
            res.json({
              success: 1,
...
router.get("/user/dashboard/", verifyToken, async function (req, res) {
  if (req.decode) {
    if (req.query.UserID) {
      ...
      await axiosInstance
        .get(api_url + "client/" + req.query.UserID)
        .then(function (responseArr) {
          console.log(responseArr.data);
...
router.put(
  "/user/dashboard/setting/:ClientID",
  verifyToken,
  async function (req, res) {
    if (req.decode) {
      console.log("Updating...");
      if (req.params.ClientID) {
        ...
        await axiosInstance
          .put(api_url + "client/" + req.params.ClientID, req.body)
          .then(function (responseArr) {
            console.log(responseArr);
            res.json({
              success: 1,
    ...
    if (req.query.LawyerID && req.query.UserID) {
      ...
      bookingObj = {
        ...
        Title:
          ...
          req.body.Email +
          "-" +
          req.body.Lawyer_FullName +
          "-" +
          req.body.Lawyer_PhoneNumber +
          "-" +
          req.body.Lawyer_Email,
        Content: content,
...
router.post("/booking", verifyToken, async function (req, res) {
  if (req.decode) {
    ...
    if (req.body.LawyerID && req.body.ClientID) {
      ...
      await axiosInstance
        .get(api_url + "lawyer/" + req.body.LawyerID)
        .then(function (responseArr) {
...
router.get("/invoice-view", verifyToken, async function (req, res) {
  if (req.decode) {
    if (req.query.BookingID) {
      ...
      await axiosInstance
        .get(api_url + "booking/" + req.query.BookingID)
        .then(function (responseArr) {
          console.log(responseArr.data);
...
router.post(
  ...
  async function (req, res) {
    if (req.file) {
      fs.readFile(req.file.path, function (err, original_data) {
        ...
        fs.writeFile(
          uploadDir +
            req.query.UserID +
            "." +
            mimeTypes.extension(req.file.mimetype),
          decodedImage,
          function (err) {}
        );
      ...
      await axiosInstance
        .put(api_url + "client/" + req.query.UserID, {
          Avatar: filepath,
        })
        .then(function (responseArr) {
          console.log(responseArr);
...
router.post(
  ...
  async function (req, res) {
    if (req.file) {
      ...
      await axiosInstance
        .put(api_url + "lawyer/" + req.query.UserID, {
          Avatar: filepath,
        })
        .then(function (responseArr) {
          console.log(responseArr);
