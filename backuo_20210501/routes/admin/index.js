...
router.post("/posts", verifyToken, async (req, res) => {
    ...
    if (req.decode){
        ...
        await axiosInstance.post('http://api.veneris.bitiland.com/api/post/', req.body)
        .then(function(response) {
...
router.get('/practice_areas', verifyToken, async (req, res) => {
    if(req.decode) {
        const axiosInstance = axios.create({
            headers: {
                Authorization: `token ${req.cookies['mylawyer']}`
...
router.delete('/lawyer/:LawyerID', verifyToken, async function (req, res) {
    if (req.decode) {
        if (req.params.GuestID) {
            const axiosInstance = axios.create({
                headers: {
            ...
            await axiosInstance.delete(api_url + 'lawyer/' + req.params.GuestID)
                .then(delResponse => {
                    res.json(delResponse.data);
                })
                .catch(err => {
